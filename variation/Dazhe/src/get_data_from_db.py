...
Description:
	...
	But afterwards the gbrowse_conf_file has to be appended to the appropriate gbrowse configuration file on the server
		where the web part of GBrowse runs. 
...
class LoadStock250kAssociationIntoGBrowseDB(object):
	...
	def parse_and_load_gff3(self, filename, feature_name, output_dir, source_name, \
						gb_host, gb_dbname, gb_db_user, gb_db_passwd,\
						percentage_threshold=10):
		"""
		2010-5-4
			make percentage_threshold an argument with default value=10
				it's an argument that determines the percentage of top association results to be put into gbrowse db.
		2010-2-4
...
""")
	...
	def run(self):
		"""
		2010-4-14
			add call_method_id to the track_name
		2010-2-4
		"""
		if self.debug:
			import pdb
		...
		import MySQLdb
		sys.stderr.write("Connecting to stock_250k, host=%s ..."%self.hostname)
		try:
		...
		while(1):
			count += 1
			sys.stderr.write("\t %s/%s  "%(count, numRows))
			...
			if exist_or_not == 0: # Data not in database yet -)
				...
				track_name = "%s Call %s By %s"%(phenotype_method_dict[phenotype_method_id], call_method_id, analysis_method_name)
				self.mk_conf(feature_name, track_name, max_score, \
				...
				sys.stderr.write("%s already in db.\n"%(feature_name))
		cursor_250k.close()
		conn_250k.close()
