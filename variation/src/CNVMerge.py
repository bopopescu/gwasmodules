...
Examples:
	# 2010-7-29 merge deletions from CNVMethod 12 to 13
...
class CNVMerge(object):
	...
	def mergeTwoSegments(cls, segment, next_segment):
		...
		new_segment = [segment[0], segment[1], stop_segment[2], segment[3], stop_segment[4],\
					no_of_probes_covered, size_affected, amplitude, numpy.mean([segment[-1], next_segment[-1]]), ]
		return new_segment
	...
	@classmethod
	def findChromosomeNeighbor(cls, i, segment_ls, direction=1, max_gap_len=10000, oneNeighborEnough=True):
		"""
		2010-7-29
	...
	def mergeOverlappingORCloseSegmentsByGraph(cls, segment_ls, max_reciprocal_gap_ratio=0.1, max_gap_len=100, \
		...
		while G.number_of_edges()>0:
			...
			try:
				...
				neighbor_set |= set(cls.findChromosomeNeighbor(i, segment_ls, direction=-1, max_gap_len=5000))	#5kb is hard-coded number
				...
				neighbor_set |= set(cls.findChromosomeNeighbor(j, segment_ls, direction=+1, max_gap_len=5000))	#5kb is hard-coded number
