...
from pymodule import figureOutDelimiter, getListOutOfStr, CNV
...
class CNVNormalize(object):
	...
	def get_input(cls, input_fname, data_type=numpy.float32):
		"""
		2010-3-18
			trunk moved to CNV.getProbeIntensityData()
		2009-10-28
		...
		2009-5-18
			become classmethod
		"""
		return CNV.getProbeIntensityData(input_fname, data_type=data_type)
	
	@classmethod
	def quantile_normalize(cls, data_matrix, no_of_probes_per_block=300000):
		"""
		2009-10-28
	...
	@classmethod
	def writeArrayXProbeHeader(cls, writer, probe_id_ls, chr_pos_ls):
		"""
		2010-5-27
			moved from MpiWithinArrayBlockQuantileNormalize.py to here.
		"""
		writer.writerow(['',''] + probe_id_ls)
		writer.writerow(['',''] + [row[0] for row in chr_pos_ls])
		writer.writerow(['',''] + [row[1] for row in chr_pos_ls])
	
	@classmethod
	def outputArrayXProbe(cls, data_matrix, probe_id_ls, chr_pos_ls, header, output_fname_prefix, split_genome_into_chromosomes=False,\
						array_id_ls=None, ecotype_id_ls=None):
		"""
		2010-5-27
			data_matrix is array by row, probe by column.
			
			probe_id_ls and chr_pos_ls are in chromosomal order.
		...
		if split_genome_into_chromosomes:
			...
			chr2start_index = {}
			old_chr = None
			for i in range(len(chr_pos_ls)):
				chr_pos = chr_pos_ls[i]
				chr = chr_pos[0]
				pos = chr_pos[1]
				if old_chr is None or chr!=old_chr:
					chr2start_index[chr] = i
					writer = csv.writer(open('%s_chr%s.tsv'%(output_fname_prefix, chr), 'w'), delimiter='\t')
					chr2writer[chr] = writer
					old_chr = chr
			writer = None
		else:
			writer = csv.writer(open('%s.tsv'%(output_fname_prefix), 'w'), delimiter='\t')
			
		no_of_arrays, no_of_probes = data_matrix.shape
	...
	@classmethod
	def outputProbeXArray(cls, data_matrix, probe_id_ls, chr_pos_ls, header, output_fname_prefix, split_genome_into_chromosomes=False,\
						array_id_ls=None, ecotype_id_ls=None):
		"""
		2010-5-27
			renamed from output().
	...
	def output(cls, data_matrix, probe_id_ls, chr_pos_ls, header, output_fname_prefix, split_genome_into_chromosomes=False,\
			array_id_ls=None, ecotype_id_ls=None, orientationType=1):
		"""
		2010-5-27
			orientationType:
				1: Array X Probe
		...
		if orientationType==1:
			cls.outputArrayXProbe(data_matrix, probe_id_ls, chr_pos_ls, header, output_fname_prefix, \
								split_genome_into_chromosomes, array_id_ls=array_id_ls, ecotype_id_ls=ecotype_id_ls)
		elif orientationType==2:
			cls.outputProbeXArray(data_matrix, probe_id_ls, chr_pos_ls, header, output_fname_prefix, \
								split_genome_into_chromosomes, array_id_ls=array_id_ls, ecotype_id_ls=ecotype_id_ls)
