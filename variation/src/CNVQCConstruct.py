...
Examples:
	...
	# Bob schmitz data
	CNVQCConstruct.py -i ~/script/variation/data/CNV/cvi_chr1_indels -s BobSchmitz2009 -t deletion -m PairedEndSolexa -o Cvi-0
		-u yh -y 3 -c
	
	# Ler-contig derived deletions
	CNVQCConstruct.py -i /tmp/Ler-deletions.tsv -s LerContigCoveredDeltion -t deletion -m PCR -o Ler-1 -u yh -y 4 -c -n 1
	
	# Ler-contig derived deletions/duplications. Deletions could also be uncovered sequences, rather than real deletions.
	# "-t deletion" is useless.
	CNVQCConstruct.py -i /tmp/Ler-copy-number.tsv -s LerContigCNV -t deletion -m PCR -o Ler-1 -u yh -y 4 -c -n1
	
	# Ler-contig span coverage over Col ref genome (the corresponding Col start-stop positions for a Ler contig)
	# it's discovered by running CNV.discoverLerContigSpanOverCol() from variation/src/misc.py
	...
	CNVQCConstruct.py -i ~/script/variation/data/CNV/Ler-span-over-Col-mdr0.4-mld50000.tsv -s LerContigCoverage
		-t unknown -m PCR -o Ler-1 -u yh -y 5 -c -n1
...
Description:
	...
	run_type 4: Ler-contig derived. First blast CNV probes against contig. Then Run CNV.discoverLerDeletionDuplication() from variation/src/misc.py
...
class CNVQCConstruct(object):
	...
	option_default_dict = {('drivername', 1,):['mysql', 'v', 1, 'which type of database? mysql or postgres', ],\
							...
							('run_type', 0, int):[1, 'y', 1, 'Run type 1: Clark et al. 2007, 2: Korbinian Schneeberger & Stephan Ossowski Paired-end, \
											3: Bob Schmitz, \
											4: Ler-contig derived, 5: Ler-contig span over Col information (table SequenceFragmentRefPos),\
	...
	@classmethod
	def getCNVQCAccessionObj(cls, session, original_id, data_source_obj):
		"""
		2009-10-28
		"""
	...
	def generateCNVQCCallObjFromLerContigDerivedCNVs(self, session, row, data_source_obj, cnv_type_obj, cnv_method_obj=None, \
		...
		"""
		2009-12-08
			each row is output of CNV.discoverLerDeletionDuplication() from misc.py
	...
	def generateCNVQCCallObjFromLerContigSpansOverCol(self, session, row, data_source_obj, cnv_type_obj=None, \
													cnv_method_obj=None, \
													original_id=None, col_name2index=None):
		"""
		2010-1-27
		...
		if sequence_fragment_obj.id:
			query = Stock_250kDB.SequenceFragmentRefPos.query.filter_by(start_probe_id=start_probe_id).filter_by(stop_probe_id=stop_probe_id).\
				filter_by(sequence_fragment_id=sequence_fragment_obj.id).filter_by(fragment_start=contig_start).filter_by(fragment_stop=contig_stop)
			if query.count()==0:
				new_obj = True
			else:
				new_obj = False
		...
		if new_obj:
			sequence_fragment_ref_pos = Stock_250kDB.SequenceFragmentRefPos(chromosome=chromosome, start=start, stop=stop, \
										...
										start_probe_id=start_probe_id, stop_probe_id=stop_probe_id, no_of_probes_covered=no_of_probes,\
										fragment_start=contig_start, fragment_stop=contig_stop)
	...
	def putQCIntoDB(self, session, input_fname, no_of_lines_to_skip, data_source_obj, cnv_type_obj, cnv_method_obj=None, \
		...
		for row in reader:
			if run_type ==1:
				db_obj = self.generateCNVQCCallObjType1(session, row, data_source_obj, cnv_type_obj, cnv_method_obj)
			elif run_type ==2:
				db_obj = self.generateCNVQCCallObjType2(session, row, data_source_obj, cnv_type_obj, cnv_method_obj)
			elif run_type==3:
				...
				db_obj = self.generateCNVQCCallObjFromLerContigSpansOverCol(session, row, data_source_obj, cnv_type_obj, \
																				cnv_method_obj=cnv_method_obj, \
																				original_id=original_id, col_name2index=col_name2index)
			elif run_type==6:
