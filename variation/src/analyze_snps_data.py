...
def run_parallel(call_method_id, x_start_i, x_stop_i):
	...
	job_id = 'ld_%s_%d_%d_%s' % (call_method_id, x_start_i, x_stop_i)
	...
	shstr += "python %sanalyze_snps_data.py %d %d %d" % \
			(env['script_dir'], call_method_id, x_start_i, x_stop_i)
...
def run_r2_calc():
	...
	if len(sys.argv) > 3:
		...
		calc_r2_levels(env['results_dir'] + '250K_r2_min01_mac15', x_start_i, x_stop_i,
			call_method_id=call_method_id, data_format=data_format)
	else:
