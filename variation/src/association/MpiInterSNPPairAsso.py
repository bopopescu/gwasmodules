...
Examples:
	...
	mpiexec ~/script/variation/src/MpiInterSNPPairAsso.py -g ~/panfs/250k/ft_gene.tsv -i ~/panfs/250k/call_method_17_binary.tsv 
		-n ~/panfs/250k/snps_context_g0_m5000_g0_m5000  -p ~/panfs/250k/phenotype.tsv -o ~/panfs/250k/boolean_snp_pair_ft_gene/ 
		-w 1-7,39-61,80-82
	...
	#
	mpiexec ~/script/variation/src/MpiInterSNPPairAsso.py -i ~/panfs/250k/call_method_29_binary.tsv 
		-n ~/panfs/250k/snps_context_g0_m10000  -p ~/panfs/250k/phenotype.tsv -o ~/panfs/250k/InterSNPBooleanKW_m10k_call29 
		-w 7 -y1  -u yh -t ~/panfs/db/results/type_1/ -l 29
...
Description:
	...
	If not given, pairwise between top 10K snps for each phenotype.
	...
	Input is StrainXSNP matrix with SNP 0 or 1 or <0(=NA). Be careful in choosing the block_size which determines the lower bound of #tests each computing node handles.
		The input node tells the computing node which genes it should work on. 
		A computing node might have to deal with lots of tests regardless of the block_size, 
		if one gene has lots of SNPs in it with #tests far exceeding the lower bound.
	...
	Option '-n', the snps context file is ignored. just supply whatever. 
"""
import sys, os, math
