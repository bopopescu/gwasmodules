...
class DrawResultPeakWorkflow(AbstractVariationWorkflow):
	...
	def run(self):
		...
		sameCategoryPhenotypeMethodLs = self.db.getPhenotypeMethodLsGivenBiologyCategoryID(self.biology_category_id, access=self.access)
		...
		for call_analysis_peak_type_id in self.call_analysis_peak_type_id_list:
			...
			result_query = self.db.getResultLs(call_method_id=call_method_id, analysis_method_id_ls=[analysis_method_id], \
			...
			result_id_ls = self.db.filterResultIDLsBasedOnResultPeak(result_id_ls, peak_type_id)
		...
		self.addJobs(workflow, db_250k=self.db, inputData=None, \
