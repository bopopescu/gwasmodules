...
def get_ecotypeid2tg_ecotypeid(curs, table='stock.ecotypeid2tg_ecotypeid', debug=False):
	...
	#rows = StockDB.EcotypeIDStrainID2TGEcotypeID.query.all()
	for row in rows:
		if is_elixirdb:
...
def get_ecotype_id_set_250k_in_pipeline(ArrayInfo):
	...
	for row in ArrayInfo.query.all():
...
def fillInPhenotypeMethodID2ecotype_id_set(PhenotypeAvgTableCalss=None):
	...
	rows = PhenotypeAvgTableCalss.query.all()
	for row in rows:
...
def fillInCallMethodID2ecotype_id_set(CallInfoTableClass=None):
	...
	rows = CallInfoTableClass.query.all()
	for row in rows:
...
def getResultForComparison(rm_x,rm_y,no_of_top_snps = 500):
	...
	for i in range(no_of_top_snps):
		data_obj = genome_wide_result_x.get_data_obj_at_given_rank(i+1)
		if not sorted_values.has_key(data_obj.chromosome):
			sorted_values[data_obj.chromosome] = {}
		sorted_values[data_obj.chromosome][data_obj.position] = {'x_value':data_obj.value,'y_value':0}
	overlap_count = 0
	for i in range(no_of_top_snps):
		data_obj = genome_wide_result_y.get_data_obj_at_given_rank(i+1)
		if not sorted_values.has_key(data_obj.chromosome):
			sorted_values[data_obj.chromosome] = {}
		if sorted_values[data_obj.chromosome].has_key(data_obj.position):
			sorted_values[data_obj.chromosome][data_obj.position]['y_value'] = data_obj.value
			overlap_count = overlap_count + 1
		else:
			sorted_values[data_obj.chromosome][data_obj.position] = {'x_value':0,'y_value':data_obj.value}
	description = {"x_value": ("number", "-log Pvalue"),"y_value": ("number", "-log Pvalue")}
	#import gviz_api
	snp_array = []
	json_result = {}
	#data_table = gviz_api.DataTable(description)
	for chr in sorted_values:
		for snp_pos,snp in sorted_values[chr].iteritems():
			snp_array.append(dict(x_value=snp['x_value'],y_value=snp['y_value'],chr=chr,pos=snp_pos))
	
	json_result['snp_array']  = snp_array
	json_result['overlap_count'] = overlap_count
	return json_result
											#,order_by="position")
