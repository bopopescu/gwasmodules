...
Examples:
	...
	%s  -i ... -l 32,80,57,75 -u yh -z banyan -s 2,3,1,3
...
class ResultPeak2AssociationLocusStat(AbstractVariationMapper):
	...
	option_default_dict.update({
							...
							('min_overlap_ratio', 1, float): ['0.1', 'm', 1, 'minimum overlap ratio, overlap length/total' ],\
		...
		for i in xrange(n):
			for j in xrange(i+1, n):
				...
				for node in peakRBDict1:
					...
					rpg.add_node(node.result_peak_id, span=segmentKey.span_ls)
					for result2_node in node_ls:
						if result2_node.result_peak_id not in rpg:
							rpg.add_node(result2_node.result_peak_id, span = [result2_node.key.start, result2_node.key.stop])
						
						rpg.add_edge(node.result_peak_id, result2_node.result_peak_id)
	...
	def run(self):
		...
		result_ls = db_250k.getResultLs(call_method_id=None, analysis_method_id_ls=[1,32], \
		...
		for result in result_ls:
			if result.call_method.locus_type_id ==1 and result.analysis_method_id==1:	#KW on snp
				result_peak_type_id = self.result_peak_type_id_ls[0]
			elif result.call_method.locus_type_id==1 and \
					(result.analysis_method_id==32 or result.analysis_method_id==8):	#EMMA on snp
				result_peak_type_id = self.result_peak_type_id_ls[1]
			elif result.call_method.locus_type_id==2 and result.analysis_method_id==1:	#deletion
				result_peak_type_id = self.result_peak_type_id_ls[2]
			elif result.call_method.locus_type_id==2 and \
					(result.analysis_method_id==32 or result.analysis_method_id==8):	#EMMA on deletion 
