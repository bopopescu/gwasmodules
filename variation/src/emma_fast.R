...
#  stopifnot(nrow(K) == t)
  ...
  if ( is.null(Z) ) {
    ...
    #a list of of all log(delta) that are going to be searched
...
emma.REMLE <- function(y, X, K, y_var, ngrids=100, llim=-10, ulim=10,
  esp=1e-6, eig.L = NULL, eig.R = NULL) {
  n <- length(y)
  t <- nrow(K)
  q <- ncol(X)
  ...
  sq_etas <- etas*etas
  Etasq <- matrix(sq_etas,n-q,m)
  LL <- 0.5*((n-q)*(log((n-q)/(2*pi))-1-log(colSums(Etasq/Lambdas)))-colSums(log(Lambdas)))
  dLL <- 0.5*delta*((n-q)*colSums(Etasq/(Lambdas*Lambdas))/colSums(Etasq/Lambdas)-colSums(1/Lambdas)) #Why times delta?  It's not in the eq. in the paper.
  ...
  for( i in 1:(m-1) )
    {
      if (dLL[i] > 0  &&  dLL[i+1] < 0) 
      {
        r <- uniroot(emma.delta.REML.dLL.wo.Z, lower=logdelta[i], upper=logdelta[i+1], tol=esp, lambda=eig.R$values, etas=etas)
        optlogdelta <- append(optlogdelta, r$root)
...
#    optdelta <- exp(optlogdelta)
  ...
  #iXX <- solve(t(X) %*% H_inverse %*% X)  #Is same as iXX above. bvilhjal-2010-05-24
  #beta <- iXX %*% t(X) %*% H_inverse %*% y
  #2008-11-12 to get the genotype variance percentage
  x_beta = X %*% beta
  x_beta_var = var(x_beta)
  var_perc = x_beta_var/y_var
  stat <-beta[q]/sqrt(iXX[q,q]*maxva)
  return (list(REML=maxLL,delta=maxdelta,ve=maxve,vg=maxva, beta=beta, var_perc=var_perc, stat=stat))
}
...
emma.ML.LRT <- function(ys, xs, K, Z=NULL, X0 = NULL, ngrids=100, llim=-10, ulim=10, esp=1e-10, ponly = FALSE) {
  ...
  q0 <- ncol(X0)	#how many betas in null model
      ...
      if ( ( mean(xv) <= 0 ) || ( mean(xv) >= 1 ) ) {	#??? mean(xv) has to be within 0 and 1
        ...
        for(j in 1:g) {	#for each phenotype
          if ( nv == t ) {	#2008-09-23 number of valid points = number of strains
            ...
            if ( is.null(Z) ) {	#no Z. assume it is diagonal identity matrix
		...
		#Null model for each marker is same if there is no missing data
        if ( ( nv == t ) && ( !ponly ) ) {	
          ML0s[i,] <- ML0
        }
...
emma.REML.t <- function(ys, xs, K, y_var, ngrids=50, llim=-4, ulim=10, esp=1e-6, debug=FALSE) {
  ...
  X0 <- matrix(1,ncol(ys),1)  #Intercept
  ...
  deltas <- matrix(nrow=m,ncol=g)
  lls <- matrix(nrow=m,ncol=g)
  ves <- matrix(nrow=m,ncol=g)
  vgs <- matrix(nrow=m,ncol=g)
  eig.L <- emma.eigen.L(NULL,K)  #EIGENVALUE

													   
  for(i in 1:m) { #For all markers
      X <- cbind(X0,xs[i,])
      eig.R1 = emma.eigen.R.wo.Z(K,X)
      
      for(j in 1:g) {
          #cat("t:", t, "\n")
          REMLE <- emma.REMLE(ys[j,],X,K,y_var[j], ngrids,llim,ulim,esp, eig.L, eig.R1)	#2008-10-05 eig.L wasn't added here before. 
          stats[i,j] <- REMLE$stat
          ...
          deltas[i,j] <- REMLE$delta
          lls[i,j] <- REMLE$REML
          vgs[i,j] <- REMLE$vg
          ves[i,j] <- REMLE$ve
      }
      ps[i,] <- 2*pt(abs(stats[i,]),n - q1,lower.tail=FALSE)
  }  
  #return (list(ps=ps,REMLs=REMLs,stats=stats,vgs=vgs,ves=ves, ve_vs_vg_ratio=ve_vs_vg_ratio, genotype_var_perc=genotype_var_perc, beta0_est=beta0_est, beta1_est=beta1_est))
  return (list(ps=ps,stats=stats,genotype_var_perc=genotype_var_perc, beta0_est=beta0_est, beta1_est=beta1_est, deltas=deltas, lls=lls, ves=ves, vgs=vgs))
  }
