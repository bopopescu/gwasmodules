...
class GWASRecord():
			...
			for pid in phed.phenIds:
				...
				try:
					...
					self._init_phenotype_(phen_name, num_vals=len(phen_vals), std_dev=sp.std(phen_vals),
						  ...
						  method_description=method_description, measurement_scale=measurement_scale,
						  is_binary=is_binary)
					self._add_phenotype_values_(phed.getPhenotypeName(pid), ecotypes, phen_vals,transformation=transformation,transformation_description=transformation_description)
	...
	def _get_analysis_methods_(self, phen_name, transformation):
		...
		try:
			...
			for x in self.h5file.iterNodes('/phenotypes/%s/%s' % (phen_name,transformation),'Group'):
				d = {'name': x._v_attrs.name, 'comment': x._v_attrs.comment}
				d['phenotype'] = phen_name
	...
	def add_results(self, phen_name, analysis_method, chromosomes, positions, scores, mafs, macs,
		...
		try:
			...
			analysis_group._v_attrs.name = analysis_method
			analysis_group._v_attrs.max_score = max(scores)
			analysis_group._v_attrs.comment = analysis_comment
			
			if analysis_method in ['emmax', 'lm']:
				table = self.h5file.createTable(analysis_group, 'results', ResultRecordLM, "Regression result")
			elif analysis_method == 'kw':
				table = self.h5file.createTable(analysis_group, 'results', ResultRecordKW, "Regression result")
			else:
...
#		table.cols.mac.createIndex()
	...
	def get_results_by_chromosome(self, phen_name, analysis_method, transformation='raw', min_mac=15, min_score=0.0, \
		...
		try:
			...
			table = info_group.results
			
			max_score = info_group._v_attrs.max_score
