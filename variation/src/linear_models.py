...
def emmax_step_wise(phenotypes, K, sd=None, num_steps=10, file_prefix=None, allow_interactions=False,
	...
	for step_i in range(1, num_steps + 1):
		emmax_res = lmm._emmax_f_test_(snps, H_sqrt_inv, snp_priors=snp_priors)
		if step_i == 1:
		...
		if snp_priors != None:
			...
			step_info['max_ppa'] = max_ppa
			step_info['max_ppa_pval'] = emmax_res['ps'][max_ppa_i]
			step_info['max_ppa_chr_pos'] = chr_pos_list[max_ppa_i]
		...
		for i, snp in enumerate(cofactor_snps):
			...
			lmm.set_factors(t_cofactors)
			if snp_priors != None:
				res = lmm._emmax_f_test_([snp], H_sqrt_inv, snp_priors=[cof_snp_priors[i]])
...
#		elif reml_res['pseudo_heritability'] < 0.01:
	...
	if snp_priors != None:
		max_ppa_i = sp.argmax(emmax_res['ppas'])
		step_info['max_ppa'] = emmax_res['ppas'][max_ppa_i]
		step_info['max_ppa_pval'] = emmax_res['ps'][max_ppa_i]
		step_info['max_ppa_chr_pos'] = chr_pos_list[max_ppa_i]
		step_info['ppa_cofactors'] = map(tuple, ppa_cofactors[:])
		ppas = emmax_res['ppas'].tolist()
