...
def get2010sequence_datas(chr, region_start, region_end, user="bvilhjal", host="papaya.usc.edu", passwd="*rri_bjarni@usc",filter_threshold=10, id=None, ref_seq_name='Col-0'):
	...
	cmd_str = """
	select distinct eva.ecotype_id, eva.accession_id, eva.nativename
	from at.alignment an, at.accession2tg_ecotypeid eva, at.sequence s """
	cmd_str += "where s.alignment=an.id and s.accession=eva.accession_id and an.start<"+str(region_end)+" and an.end>"+str(region_start)+" and an.chromosome="+str(chr)+" "
	cmd_str += """
	...
	cmd_str += "where s.alignment=an.id and an.start<"+str(region_end)+" and an.end>"+str(region_start)+" and an.chromosome="+str(chr)+" "
	cmd_str += """
	order by an.start
	"""
	numRows = int(cursor.execute(cmd_str))
	...
	for (start_pos, end_pos) in alignment_positions:
		...
		cmd_str += "where l.alignment=an.id and an.start="+str(start_pos)+" and an.end="+str(end_pos)+" and an.chromosome="+str(chr)+" and l.offset>0 "
		cmd_str += """
...
def _test_():
	...
	alignment_datas = get2010sequence_datas(5, 0, 35000000,filter_threshold=0)
	for ad in alignment_datas:
		ad.write_to_file_quan_format("/tmp/quan_2010_chr5.csv")
